...
export function MarketplacePage() {
  ...
  return <main className="market"><Header/><header><span className="eyebrow">MARKETPLACE</span><h1>Components for medical AI research.</h1><p>Compare tools, skills, and interfaces with their public source context.</p><label className="search-label" htmlFor="search">Search components</label><input id="search" type="search" placeholder="Search names, descriptions, or tags…" value={params.get("q") ?? ""} onChange={e=>update("q", e.target.value)} /></header>
    <div className="directory-layout"><aside className="filter-panel" aria-label="Catalog filters"><h2>Browse the catalog</h2>{filters.map(f=><label key={f.key}>{f.label}<select value={params.get(f.key) ?? ""} onChange={e=>update(f.key,e.target.value)}><option value="">All</option>{f.values.map(v=><option key={v} value={v}>{v}</option>)}</select></label>)}<button onClick={()=>setParams({})}>Clear filters</button><p>Stable and Candidate identify source tiers; neither is a verification badge.</p></aside>
