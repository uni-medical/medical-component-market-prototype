...
export function EcosystemShowcase({ catalog, locale }: { catalog: CatalogIndex; locale: Locale }) {
  ...
  return <main className={styles.page} aria-labelledby="ecosystem-title">
    ...
    <section className={styles.section} aria-labelledby="ecosystem-domains-title"><div className={styles.sectionHead}><div><span className={styles.kicker}>01 / {text.domains}</span><h2 id="ecosystem-domains-title">{text.domains}</h2><p>{text.domainsNote}</p></div><span className={styles.sectionMark}>10 × topics</span></div><div className={styles.domainGrid}>{domains.map((domain, index) => <article className={`${styles.domainCard} ${styles[`accent${index}`]}`} key={domain.id}><span>0{index + 1}</span><h3>{locale === "zh" ? ({ "Medical research": "医疗研究", "Agent infrastructure": "Agent 基础设施", "MCP & integration": "MCP 与集成", "Scientific discovery": "科学发现", "Developer tooling": "开发者工具链" } as Record<string,string>)[domain.label] ?? domain.label : domain.label}</h3><p>{domain.description}</p><strong>{domain.count} {text.records}</strong></article>)}</div></section>
